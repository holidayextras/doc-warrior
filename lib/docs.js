...
docWarrior.prototype.get = function(opts, callback){
  var self = this;
  if(!opts) return callback('opts not passed to get');
  if(!opts.docs) return callback('docs not passed within opts to get');
...
docWarrior.prototype._processDocument = function(docType, date, params, callback){
  var self = this;
  if(!docType) return callback('No docType passed to _processDocument');
...
docWarrior.prototype._getDocument = function(docType, date, callback){
  if(!docType) return callback('No docType passed to _getDocument');
  connector.getDocument(docType, date, callback);
};
