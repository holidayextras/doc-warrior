...
database.prototype.getDocument = function(doc, date, callback){
  ...
  */
  ...
  var query = 'SELECT type, MAX(date) AS date, \
    (SELECT rules FROM ' + tabelName + ' \
      v2 WHERE v2.type = v.type AND v2.date = max(v.date) LIMIT 1) AS rules, \
    (SELECT content FROM ' + tabelName + ' \
      v2 WHERE v2.type = v.type AND v2.date = max(v.date) LIMIT 1) AS content \
    FROM ' + tabelName + ' v \
    WHERE type LIKE ? \
    AND date <= ? \
    GROUP BY type;'
